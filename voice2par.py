from evPar import speech_to_text, get_volume_times, otnosh, get_monotone, get_tone_balance, get_intfrnce_distn, get_noise, get_intenference
...
class Sound2VecModel:
 ...
 #ws1.cell(column=1, row=4, value="Volume:")
                ...
                print('Отношение тишина-речь: ', self.silens_segm)


                self.nnoise = self.get_nnoise(one_channel_path)
                print('Отношение речь-шум:', self.nnoise)
                ...
                print('Темп: ', self.text)

                self.distnt=self.get_distnt(one_channel_path)
                print('Пространственное впечатление: ', self.distnt)

                self.tonebal = self.get_tonebal(one_channel_path)
                print('Тональный баланс: ', self.tonebal)

                self.audiointer = self.get_audiointer(one_channel_path)
                print('Заметность помех и искажений: ', self.audiointer)
    ...
    def __str__(self):
        if self.is_speech:
            return f'{self.result} {none_to_empty(self.volume)} {none_to_empty(self.silens_segm)} {none_to_empty(self.nnoise)} {none_to_empty(self.monotone)} {none_to_empty(self.text)} {none_to_empty(self.distnt)} {none_to_empty(self.tonebal)} {none_to_empty(self.audiointer)}'
        else:
    ...
    def get_nnoise(self, audio_path):
        return get_noise(audio_path)

    def get_distnt(self, audio_path):
        return get_intfrnce_distn(audio_path)

    def get_tonebal(self, audio_path):
        return get_tone_balance(audio_path)

    def get_audiointer(self, audio_path):
        return get_intenference(audio_path)
